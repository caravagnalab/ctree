...
#'
#' @description Create or compute phylogenies for REVOLVER fit from CCF data. This method should be used also if one
...
#'       store.max = 100)
#     )
ctree_sampler = function(CCF_clusters,
                         drivers,
                         samples,
                         patient,
                         sspace.cutoff = 10000,
                         ...
                         store.max = 100)
{
  ...
  else
  {
    ...
    pio::pioTit("Building clone trees, per region (this might take some time)")
    ...
    pio::pioDisp(data.frame(
      region = names(clonevol.obj$models),
      Trees = numSol,
  ...
  # =-=-=-=-=-=-=-=-=-=-=-=-=-=
  pio::pioTit("Creating `ctree` objects")
  
  # Special case: there are none
  if (length(TREES) == 0)
  {
    message('No trees for this patient -- check data and input? -- returning NULL\n')
...
are_suitable_precomputed_trees = function(x,
                                          patient,
                                          ...
                                          precomputed.scores = NULL)
{
  ...
  OK_size = (cmt_size == rmt_size) &&
    (cmt_size == length(required_clusters))
  
  if (!all(OK_size)) {
    message(
      "Input trees must be NxN adjacency matrices because this patient has N = ",
      length(required_clusters),
      " clusters. Some are not."
    )
  ...
  OK_clusters = sapply(precomputed.trees,
                       function(w) {
  ...
  if (!all(OK_clusters)) {
    message(
      "Input trees must contain entries for all clusters ",
      paste(required_clusters, collapse = ', '),
      " but some are not."
    )
  ...
  if (!all(OK_trees)) {
    message(
      "Input trees are not trees and might have multiple roots, disconnected components etc."
    )
